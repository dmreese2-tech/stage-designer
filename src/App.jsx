...
const DEFAULT_VENUE = {
  ...
  roomW: 60, roomH: 55,
  stageOffX: 10, stageOffY: 12,
  roomShape: "rect",        // "rect" | "poly"
  roomPoints: [],           // for poly mode
  audience: [],             // [{ x,y,w,h,label,seats,color }]
  showRoom: true,
  showAudience: true,
  roomColor: "#111118",
  ...
  houseSeats: 100,
  orientation: "N",         // which wall is upstage: N=top, S=bottom, E=right, W=left
  vomitories: [],           // [{ wall:"N"|"S"|"E"|"W", pos:0..1, w:ft }]
};
...
export default function StageDesigner() {
  ...
  const [stageW, setStageW] = useState(40);
  const [stageH, setStageH] = useState(30);
          ...
          <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
            ...
            <div style={{ width:220, borderLeft:`1px solid ${COLORS.border}`, padding:14, background:COLORS.bg, display:"flex", flexDirection:"column", gap:10, flexShrink:0 }}>
              ...
              <svg width={pW} height={pH} style={{ border:`1px solid ${COLORS.border}`, borderRadius:4, display:"block", maxWidth:"100%" }}>
                ...
                {/* Stage */}
                <rect x={sX} y={sY} width={sSW} height={sSH} fill={COLORS.stage} stroke={COLORS.stageEdge} strokeWidth={2}/>
        ...
        <div style={{ flex:1, position:"relative", overflow:"hidden", background:COLORS.bg }}
          ...
          <svg ref={svgRef} style={{ width:"100%", height:"100%", display:"block",
            ...
            <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
              ...
              {/* Stage floor */}
              <rect x={0} y={0} width={SW} height={SH} fill={COLORS.stage} stroke={COLORS.stageEdge} strokeWidth={3}/>
              {/* Plasterline */}
              <line x1={0} y1={0} x2={SW} y2={0} stroke={COLORS.stageEdge} strokeWidth={2.5} strokeDasharray="12,6"/>
              {/* Center line */}
              <line x1={SW/2} y1={0} x2={SW/2} y2={SH} stroke={COLORS.stageEdge} strokeWidth={1} strokeDasharray="6,4" opacity={0.4}/>
              {gridLines}
              {measureLabels}
              <text x={SW/2} y={SH/2} fill={COLORS.stageEdge} fontSize={18} textAnchor="middle" dominantBaseline="middle" opacity={0.25} letterSpacing={3}>STAGE FLOOR</text>
              <text x={SW/2} y={SH/2+24} fill={COLORS.stageEdge} fontSize={11} textAnchor="middle" dominantBaseline="middle" opacity={0.18}>{stageW}' × {stageH}'</text>
              {/* DS label */}
              <text x={SW/2} y={SH+18} fill={COLORS.stageEdge} fontSize={10} textAnchor="middle" opacity={0.5}>▲ DOWNSTAGE / AUDIENCE</text>
